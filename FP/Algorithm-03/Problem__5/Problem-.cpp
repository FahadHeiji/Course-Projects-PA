...
void SumColMatrix(int arr[3][3], int rows, int cols, int SumColArr[3])
{
    cout << "\nHere is The Sum of Cols Matrix:" << endl;
    for (short col = 0; col < cols; col++)
    {
        SumColArr[col] = SumColMatrix(arr, col, rows);
    }
}
void PrintSumColMatrix(int arr[3][3], int rows, int cols, int SumColArr[3])
{
    ...
    for (short col = 0; col < cols; col++)
    {
        cout << setw(2) << "Col " << col + 1 << " Sum = " << SumColArr[col] << endl;
    }
}
int main()
{
    ...
    PrintMatrix(arr);
    SumColMatrix(arr, 3, 3, SumColArr);
