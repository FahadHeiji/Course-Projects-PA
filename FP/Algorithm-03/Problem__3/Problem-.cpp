...
short SumRowMatrix(int arr[3][3], int rowNumber, int cols, int SumRowArr[3])
{
    short sum = 0;
    for (short col = 0; col < cols; col++)
    {
    ...
    return SumRowArr[rowNumber] = sum;
}
...
void PrintSumRowMatrix(int arr[3][3], int rows, int cols, int SumRowArr[3])
{
    cout << "\nHere is The Sum of Rows Matrix:" << endl;
    for (short row = 0; row < rows; row++)
    {
        cout << setw(2) << "Row " << row + 1 << " Sum = " << SumRowMatrix(arr, row, cols, SumRowArr) << endl;
    }
}
int main()
{
    ...
    PrintMatrix(arr);
    PrintSumRowMatrix(arr, 3, 3, SumRowArr);
    return 0;
}